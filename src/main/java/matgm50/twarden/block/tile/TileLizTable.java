...
public class TileLizTable extends TileEntity implements IInventory {
    ...
    @Override
    public String getInventoryName() {return BlockLib.LIZBETH_TABLE_INV_NAME;}
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        ...
        if(!stack.isStackable() && stack.isItemStackDamageable() && stack.getItem().isRepairable()) {
            return true;
        } else if(stack.getItem() instanceof ItemWardenArmor || stack.getItem() instanceof ItemWardenSword) {
            return true;
        } else {
            return false;
        }
    ...
    public void Repair(int slot) {
        ...
        if(toRepair != null) {
            ...
            if(toRepair.getItem().isDamageable() && toRepair.getItemDamage() != 0) {
    ...
    public void infuse(int slot, Aspect aspect) {

        ItemStack toInfuse = inv[slot];

        if(toInfuse != null) {

            WardenicHelper.setUpgradeOnStack(inv[slot], aspect.getName());

            worldObj.playSoundEffect(xCoord + 0.5D, yCoord + 0.5D, zCoord + 0.5D, "thaumcraft:wand", 1.0F, worldObj.rand.nextFloat() * 0.1F + 0.9F);
